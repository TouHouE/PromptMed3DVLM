...
class ViT(nn.Module):
    ...
    def forward(self, x, return_hidden_states: bool = False, **kwargs):
        ...
        hidden_states_out = [x]
        for blk_id, blk in enumerate(self.blocks, 1):
            x = blk(x)
            if blk_id % (self.num_layers // self.num_block) == 0:
                hidden_states_out.append(x)
        x = self.norm(x)        
        hidden_states_out[-1] = x
...
if __name__ == '__main__':
    ...
    out, hidden_states = model(x, return_hidden_states=True)
    print(out.shape)
    for layer_order, state in enumerate(hidden_states):
        print(f'layer order: {layer_order}, shape: {state.shape}')
    print('done')